...
#' 
#' Modified from ck37r/sl_mgcv.R - add back to package when it's working well.
#'
...
#'   classification
#' @param nthreads Number of cores to use, default 1.
#'
#' @param degree Degrees of freedom for the splines, similar to # of knots but with penalization.
...
#' @param basis_fn Basis function for the splines, default of "tp". "cr" for cubic regression.
...
#' @param select If covariates can be removed entirely due to a penalty, default TRUE.
...
#' @export
SL.mgcv2 = function(Y, X, newX, family,
                   obsWeights = NULL, id = NULL,
                   degree = -1,
                   nthreads = 1L,
                   continuous_values = 5L,
                   smooth_fn = "s",
                   basis_fn = "tp",
                   exclude_spline = NULL,
                   method = "REML",
...
get_mgcv_formula = function(X, exclude_spline, continuous_values,
                            smooth_fn, degree, basis_fn) {
